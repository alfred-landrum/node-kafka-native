...
// default transformation for received kafka messages
// before issuing the receive callback.
var default_kmsg_mapper = function(kmsg) {
    return JSON.parse(kmsg.payload);
};
...
function Consumer(options) {
    ...
    // Worker info: if used, then 0 <= worker_id < num_workers must hold,
    // and each worker will calculate a fair share of partitions to consume.
    self.worker_id = options.worker_id || 0;
    self.num_workers = options.num_workers || 1;
    assert(self.worker_id >= 0);
    assert(self.worker_id < self.num_workers);
    ...
    self.kmsg_mapper = options.kmsg_mapper || default_kmsg_mapper;
    ...
    this.handle = new RawConsumer({
        ...
        driver_options: _.defaults({}, options.driver_options, {
            ...
            'queued.max.messages.kbytes': options.queued_max_messages_kbytes,
...
Consumer.prototype._rdkakfa_recv_callback = function(kmsgs) {
    ...
    for (var i = 0; i < kmsgs.length; ++i) {
        ...
        try {
            messages.push(self.kmsg_mapper(kmsg));
        } catch (err) {
            self.logger.error('kmsg_mapper failed: ', err);
        }
...
Consumer.prototype._rdkafka_stats_callback = function(statstr) {
    ...
    var waiting_memory = partitions[partition_ids[0]].fetchq_cnt;

    var waiting_disk = _.sumBy(partition_ids, function(p) {
    ...
    var disk_messages = _.sumBy(partition_ids, function(p) {
    ...
    this.options.stats_callback({
        waiting: waiting_memory + waiting_disk,
